 ...
 *
 * Why ghost_underlying instead of ERC20.balanceOf():
 ...
 *
 *   QIEVaultHarness overrides totalAssets() to read ghost_underlying
 *   (possible because we added `virtual` to the production function).
 *   The formula is identical: ghost_underlying + 1.
 *   The spec bounds ghost_underlying to a realistic value.
 */
...
// Bounding ghost_underlying here ensures the prover does not explore
 ...
 *
 * Bound rationale:
 *   ghost_underlying <= 1e18 excludes states where totalAssets >>
 ...
 * itself (the caller does that before invoking it). ghost_underlying
 ...
 * ghost_underlying > 0). Classic ERC4626 inflation attack setup.
 *
